...
let rec evalue x polynome =
  match polynome with
  | [] -> 0.
  | t::q -> t +. x *. (evalue x q)

let evalue x polynome =
  List.fold_right (fun tete resultat_queue -> tete +. x *. resultat_queue) polynome 0.
...
let rec retract liste =
    let rec aux_reste_que_zero liste bol =
      match liste with
      | [] -> bol
      | t::q -> if t = 0. then aux_reste_que_zero q bol else false
    in 
      match liste with
      | [] -> []
      | t::q -> if t = 0. && aux_reste_que_zero q true then [] else t::(retract q) 
...
let scal_mult scal polynome =
    if scal = 0. then
      []
    else
      List.map (fun t -> t*.scal) polynome
...
let plus poly1 poly2 =
  List.map2 (fun t1 t2 -> t1+.t2) poly1 poly2

let rec plus poly1 poly2 =
  retract (match poly1,poly2 with
  | [],[] -> []
  | [],t::q -> t::q
  | t::q,[] -> t::q
...
*)
let rec parcours_arbre  entier arbre_econd =
  let rec aux_decoder liste acc =
    match arbre_econd with
  | Vide -> acc
  | Lettre (a) -> a::acc
  | Noeud (b1,b2,b3) ->
      let (premier::reste_code) = decompose_int entier in
        match premier with
        | 1 -> 
            (match b1 with
            | Vide -> raise CodeNonValide
            | Lettre (a) -> a::acc
            | Noeud (b1,b2,b3) -> parcours_arbre (recompose_int reste_code) b1)
        | 2 ->
           ( match b2 with
            | Vide -> raise CodeNonValide
            | Lettre (a) -> a::acc
            | Noeud (b1,b2,b3) -> parcours_arbre (recompose_int reste_code)  b2)
        | 3 ->
            (match b3 with
            | Vide -> raise CodeNonValide
            | Lettre (a) -> a::acc
            | Noeud (b1,b2,b3) -> parcours_arbre (recompose_int reste_code)  b3)
        | _ -> raise CodeNonValide
  | _ -> failwith "Erreur"
  in aux_decoder (decompose_int entier) []
