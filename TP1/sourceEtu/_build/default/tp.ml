...
let ieme (x,y,z) i = match i with
   | 1 -> x
   | 2 -> y
   | 3 -> z
   | _ -> failwith "i doit être entre 1 et 3"
...
(* Not genuine *)
let  padovan2 n = 
   let rec padovan_acc n acc1 acc2 acc3 =
      if n = 0
         then acc3
      else
         padovan_acc (n-1) (acc3+acc2) acc1 acc2 in padovan_acc n 1 0 0;;
...
let estPremier n = 
   let rec estPremier_aux n i = 
      if n = i
         then true
      else if n mod i = 0
         then false
      else
         estPremier_aux n (i+1) in
   if n = 0 || n = 1
      then false
   else
      estPremier_aux n 2
