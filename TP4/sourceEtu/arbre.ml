...
let rec retrait_arbre element (Noeud (b, lb)) = match element with
   | [] -> Noeud(false,lb)
   | tc::qc ->
      let arbre_c =
         ...
         | None   -> Noeud (false, [])
         | Some a -> a
       in Noeud (b, maj tc (retrait_arbre qc arbre_c) lb)



let rec parcours (Noeud (b,liste_fils)) =
   match (b,liste_fils) with
   | (true,[]) -> [[]]
   | (false,[]) -> []
   | _ ->
         (* (List.map (fun li -> t::li) (parcours a) @ (parcours (Noeud(false, fils)))) *)
         let result = 
            List.flatten (List.map (
               fun (cha,sous_fils) ->
                  let res = parcours sous_fils
               in match res with
                  | [] -> []
                  | _ -> List.map (fun sl -> cha::sl) res
            ) liste_fils) in if b then []::result else result


let normalise arbre = 
   List.map (fun li -> ajout_arbre li (Noeud(true,[]))) (parcours arbre)
